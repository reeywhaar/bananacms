  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            {visible.map((item) => (
              <SortableBlockRow
                ...
                item={item}
                depth={
                  item.id === activeId && projection ? projection.depth : item.depth
                }
              />
            ))}
          </SortableContext>
        ...
        {visible.length === 0 && (
          <div className="text-sm italic opacity-50">No blocks.</div>
        )}
      </div>
...
const SortableBlockRow: FC<{ item: FlatItem; depth: number }> = ({ item, depth }) => {
  ...
  return (
    ...
    >
      ...
      <span className="uppercase tracking-wide text-gray-400 w-12 shrink-0">
        {content.type}
      </span>
      <span className="font-mono text-gray-700 w-28 truncate shrink-0">
        {content.key || '—'}
      </span>
