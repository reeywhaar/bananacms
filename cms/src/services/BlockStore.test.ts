...
describe('BlockStore.updateContent', () => {
  it("replaces a block's content, and leaves its place and the other blocks as they were", async () => {
    using testDb = await createTestDb()
    await seedHostPostWithBlocks(testDb)
    const store = new BlockStore(testDb.db)
    await store.updateContent(TEXT_BLOCK, {
      type: 'text',
      key: 'intro',
      contentType: 'plain',
      text: 'Hello again',
    })
    const blocks = await store.query().parentedBy({ table: 'post', id: POST_ID }).all()
    expect(blocks.find((b) => b.id === TEXT_BLOCK)?.content).toMatchObject({
      text: 'Hello again',
    })
