...
export const GET = createRouteHandler<{ params: Promise<{ id: string; hash: string }> }>(
  async (req: NextRequest, { params }) => {
    ...
    const outputAs: AssetOutputFormat = asset.content?.output_as ?? { type: 'original' }
    const sourceRes: AssetResolution = asset.content?.resolution ?? '@1x'
    const maxSize = asset.content?.maxSize
    ...
    const effectiveRes: AssetResolution =
      resFactor[res] > resFactor[sourceRes] ? sourceRes : res
    const effectiveHash =
      effectiveRes === res
        ? hash
        : assetVariantHash(id, outputAs, effectiveRes, sourceRes, maxSize)
    if (effectiveRes !== res) {
