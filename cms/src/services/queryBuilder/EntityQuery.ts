...
> {
  ...
  protected constructor(db: Db, state: Readonly<TState>) {
    this.db = db
    this.state = state
  }
  ...
  /**
   * Return the 0-based position of the anchor row (identified by the `fn`
   * callback) within the current query's ordered result set, without applying
   * any limit/offset.  Returns -1 if the anchor row does not exist.
   *
   *   const idx = await postStore.query()
   *     .inCategory({ id })
   *     .orderBy('position')
   *     .indexOf(q => q.byShortId(shortId))
   *
   *   const next = await postStore.query()
   *     .inCategory({ id })
   *     .orderBy('position')
   *     .offset(idx + 1).first()
   */
  abstract indexOf(fn: (q: this) => this): Promise<number>

  async first(): Promise<TRow | null> {
    const rows = await this.clone({ limit: 1 } as Partial<TState>).all()
    return rows[0] ?? null
  }
