...
import type { BackupConfig, BackupMode } from './config.ts'
import { BackupPusher } from './pusher.ts'
...
describe('BackupPusher', () => {
  ...
  it('runs one pass at a time', async () => {
    ...
    const pusher = new BackupPusher({ config })
    // the second waits for the first, and finds nothing written since
    expect(await Promise.all([pusher.runOnce(), pusher.runOnce()])).toEqual(['pushed', 'quiet'])
    expect(agent.uploads).toHaveLength(1)
    await pusher.stop()
  })

  it('lets the pass under way finish when stopped', async () => {
    const { config, agent } = await setup()
    const pusher = new BackupPusher({ config })
    const pass = pusher.runOnce()
    await pusher.stop()
    expect(await pass).toBe('pushed')
    expect(agent.uploads).toHaveLength(1)
  })
