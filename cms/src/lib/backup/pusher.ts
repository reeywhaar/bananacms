...
export type PassResult =
  ...
  | 'quiet'
  /**
   * There is no database yet: the server creates it on its first request, and
   * this loop starts with the server, so on a fresh data directory its first
   * pass comes first.
   */
  | 'not-ready'
...
export class BackupPusher {
  ...
  /**
   * The pass under way, which the next one waits for: passes share the
   * connections and the recorded state, and stop() lets the last one finish
  ...
  constructor(opts: BackupPusherOptions) {
    ...
    // While there's no database yet, look again shortly rather than waiting out
    // a full interval: a fresh site is the one most worth having a copy of.
  ...
  /**
   * Stops the loop, and closes the connections it holds once the pass under
   * way, if there is one, has finished.
   */
  ...
  /**
   * Builds a copy, sends it if it is due, and remembers what was sent, once the
   * pass under way, if there is one, has finished.
   *
   ...
   * counter consulted.
   */
  runOnce(options: { force?: boolean } = {}): Promise<PassResult> {
    const pass = this.passing.then(() => this.pass(options))
    this.passing = pass.catch(() => {})
    return pass
  }

  private async pass({ force = false }: { force?: boolean }): Promise<PassResult> {
    ...
    // anything: an agent told how often to expect an archive can say a site has
    // stopped backing up, and with copies arriving only when somebody edits
    // something it cannot tell a broken site from a quiet one.
    const period = backupPeriodMs(this.config.mode)
    const due = period > 0 && state !== null && now.getTime() - state.pushedAt.getTime() >= period

    // The cheap question first. The database holds the assets' files, so
    // vacuuming it to hash it on every pass would copy the site's whole media
    // library every five minutes to find out whether anybody touched a post.
    // `PRAGMA data_version`
