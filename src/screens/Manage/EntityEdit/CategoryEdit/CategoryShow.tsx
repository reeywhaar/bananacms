...
import { PostReorderList } from './PostReorderList'

export default async function CategoryShow({ id }: { id?: string }) {
  if (!id) notFound()

  const services = await getServices()
  const category = await new CategoryStore(services.db).get(id)
  if (!category) notFound()

  const posts =
    (await new PostStore(services.db).getByParent({ table: 'category', column: 'id', value: id })) ?? []

  const items = posts.map((post) => ({
    id: post.id,
    name: post.name,
    url: routing.entityEdit('post', post.id),
    status: post.status,
  }))
