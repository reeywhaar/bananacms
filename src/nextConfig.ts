...
export function cmsRewrites(cms: CMSInstance = getCMS()): Rewrite[] {
  const url = cms.env.cmsInternalUrl.replace(/\/+$/, '')
...
export function createConfig(amend: (base: NextConfig) => NextConfig = (b) => b): NextConfig {
  ...
  const hostname = (() => {
    try {
      return new URL(cms.env.publicUrl).hostname
    } catch {
      return undefined
    }
  })()
...
export function createCmsZoneConfig(amend: (base: NextConfig) => NextConfig = (b) => b): NextConfig {
  const cms = getCMS()
  const base: NextConfig = {
    allowedDevOrigins: [
      ...(process.env.ALLOWED_HOSTS?.split(',') ?? []),
