...
import { intoResult } from '../utils/result.ts'
import { LocalizationStore, type Translations } from './LocalizationStore.ts'
import { AttributeStore } from './AttributeStore.ts'
import { EntityQuery, type BaseQueryState, type SortOrder } from './queryBuilder/EntityQuery.ts'
import {
  assetExistsClause,
  attributeExistsClause,
  type AssetSpec,
  type AttributeSpec,
} from './queryBuilder/predicates.ts'
...
export class BlockStore {
  ...
  // Replaces what a block that isn't a group holds, in the default locale, as a
  // site does with the data it keeps in one, like a poll's counts. Its parent,
  // position, attributes and translations stay as they are.
  async updateContent(id: string, content: Exclude<BlockType, BlockTypeGroup>): Promise<void> {
    await this.db
      .update(block)
